...
const { findOneInfluencerByPhoneOrEmail, findOneInfluencerById } = require("../../salesforce/influencer/findOneInfluencer.js");
const { createInfluencer} = require("../../salesforce/influencer/createInfluencer.js"); 
...
router.post(
    ...
    async (req, res) => {
        ...
        const { influencerName, influencerCategory, influencerEmail, influencerPassword, influencerPhone } = req.body;

        try{
            // Check if the influencer already exists in the database
            influencer = await findOneInfluencerByPhoneOrEmail(influencerPhone, influencerEmail, authCache);

            if (influencer) {
                return res.status(400).json({ errors: [{ msg: "Influencer with this phone/email already exists" }] });
            }
            ...
            result = await createInfluencer(influencerName, influencerCategory, influencerEmail, hashedPassword, influencerPhone);
...
router.post(
    ...
    async (req, res) => {
        ...
        try {
            ...
            let influencer = await findOneInfluencerByPhoneOrEmail(null, influencerEmail);
            ...
            const isMatch = await bcrypt.compare(influencerPassword, influencer.PasswordHash__c);
